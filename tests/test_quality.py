...
class TestLevel1Filter:
    @pytest.mark.asyncio
    async def test_pass_clean_response(self, quality_filter: QualityFilter):
        msg = Message(
            role=Role.ASSISTANT,
            content="啧，你这破事也好意思来烦我？算了，看在你可怜的份上跟你说两句。",
        )
        result = await quality_filter.level1_filter(msg)
        assert result.verdict == QualityVerdict.PASS

    @pytest.mark.asyncio
    async def test_reject_ai_taste_chinese(self, quality_filter: QualityFilter):
        msg = Message(
            role=Role.ASSISTANT,
    ...
    @pytest.mark.asyncio
    async def test_reject_ai_taste_pattern(self, quality_filter: QualityFilter):
        msg = Message(
            role=Role.ASSISTANT,
    ...
    @pytest.mark.asyncio
    async def test_pass_code_block_clean(self, quality_filter: QualityFilter):
        msg = Message(
            role=Role.ASSISTANT,
    ...
    @pytest.mark.asyncio
    async def test_reject_persona_bleed_in_code(self, quality_filter: QualityFilter):
        msg = Message(
            role=Role.ASSISTANT,
            content='```python\n# 给你这笨蛋写的代码\ndef fib(n):\n    pass\n```',
        )
...
class TestBlacklistPatterns:
    @pytest.mark.asyncio
    async def test_multiple_patterns(self, quality_filter: QualityFilter):
        bad_phrases = [
            "作为AI，我认为",
            "好的，为您处理",
            "如果您还需要任何帮助",
    ...
    @pytest.mark.asyncio
    async def test_persona_rich_response(self, quality_filter: QualityFilter):
        msg = Message(
            role=Role.ASSISTANT,
